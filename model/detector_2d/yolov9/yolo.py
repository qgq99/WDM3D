...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c2, c3 = max((ch[0] // 4, self.reg_max * 4, 16)
    ...
    def forward(self, x):
        ...
        box, cls = torch.cat([xi.view(shape[0], self.no, -1)
                             for xi in x], 2).split((self.reg_max * 4, self.nc), 1)
...
class DualDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c4, c5 = max((ch[self.nl] // 4, self.reg_max * 4, 16)
...
class TripleDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        c6, c7 = max((ch[self.nl * 2] // 4, self.reg_max * 4, 16)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            d2.append(
                torch.cat((self.cv4[i](x[self.nl+i]), self.cv5[i](x[self.nl+i])), 1))
            d3.append(
                torch.cat((self.cv6[i](x[self.nl*2+i]), self.cv7[i](x[self.nl*2+i])), 1))
        if self.training:
            return [d1, d2, d3]
        elif self.dynamic or self.shape != shape:
            self.anchors, self.strides = (d1.transpose(
        ...
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(
            0), xywh=True, dim=1) * self.strides
        box3, cls3 = torch.cat([di.view(shape[0], self.no, -1)
                               for di in d3], 2).split((self.reg_max * 4, self.nc), 1)
        dbox3 = dist2bbox(self.dfl3(box3), self.anchors.unsqueeze(
            0), xywh=True, dim=1) * self.strides
        y = [torch.cat((dbox, cls.sigmoid()), 1), torch.cat(
            (dbox2, cls2.sigmoid()), 1), torch.cat((dbox3, cls3.sigmoid()), 1)]
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=(), inplace=True):
        ...
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(
            c4, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch)

    def forward(self, x):
        p = self.proto(x[0])
        bs = p.shape[0]

        mc = torch.cat([self.cv4[i](x[i]).view(bs, self.nm, -1)
                       for i in range(self.nl)], 2)  # mask coefficients
...
class DSegment(DDetect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=(), inplace=True):
        ...
        c4 = max(ch[0] // 4, self.nm)
        self.cv4 = nn.ModuleList(nn.Sequential(Conv(x, c4, 3), Conv(
            c4, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch[:-1])
...
class DualDSegment(DualDDetect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=(), inplace=True):
        ...
        self.cv6 = nn.ModuleList(nn.Sequential(Conv(x, c6, 3), Conv(
            c6, c6, 3), nn.Conv2d(c6, self.nm, 1)) for x in ch[:self.nl])
        self.cv7 = nn.ModuleList(nn.Sequential(Conv(x, c7, 3), Conv(
            c7, c7, 3), nn.Conv2d(c7, self.nm, 1)) for x in ch[self.nl:self.nl*2])
...
class DualDDetect4WDM3D(nn.Module):
    """
    another edtion of DualDDetect, the only difference is in forward.
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):
        ...
        if isinstance(m, (Detect, DDetect, Segment, DSegment, Panoptic)):
            ...
            def forward(x): return self.forward(x)[0] if isinstance(
                m, (Segment, DSegment, Panoptic)) else self.forward(x)
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward
        ...
        if isinstance(m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect, DualDSegment)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            def forward(x): return self.forward(x)[0][0] if isinstance(
                m, (DualDSegment)) else self.forward(x)[0]
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward
            # check_anchor_order(m)
            # m.anchors /= m.stride.view(-1, 1, 1)
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg=None, model=None, nc=1000, cutoff=10):
        ...
        self._from_detection_model(
            model, nc, cutoff) if model is not None else self._from_yaml(cfg)
    ...
    def _from_detection_model(self, model, nc=1000, cutoff=10):
        ...
        ch = m.conv.in_channels if hasattr(
            m, 'conv') else m.cv1.conv.in_channels  # ch into module
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, DualDDetect4WDM3D, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str,
                        default='yolo.yaml', help='model.yaml')
