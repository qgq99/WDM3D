...
class Calibration(object):
    ...
    def img_to_rect(self, u, v, depth_rect):
        ...
        :return:
        """
        x = ((u - self.c_u) * depth_rect) / self.f_u + self.b_x
        y = ((v - self.c_v) * depth_rect) / self.f_v + self.b_y
    ...
    def affine_transform(self, img_size, trans):
        ...
        new_calib_matrix[-1, -1] = self.P[-1, -1]
        new_calib_matrix = new_calib_matrix.T
        new_calib_matrix[2, 2] = 1

        self.P = new_calib_matrix
        self.c_u = self.P[0, 2]
        self.c_v = self.P[1, 2]
        self.f_u = self.P[0, 0]
        self.f_v = self.P[1, 1]
...
def init_bev_image(out_size=768):
    bird_view = np.ones((out_size, out_size, 3), dtype=np.uint8) * 242
...
def show_horizon_heatmap(vis_target, horizon_heat_map):
    horizon_vis_img = vis_target.get_field('horizon_vis_img').numpy().astype(np.uint8)
    pad_size = vis_target.get_field('pad_size').numpy()
    horizon_heat_map = horizon_heat_map[0]
    ground_plane = vis_target.get_field('ground_plane').numpy()
    calib = vis_target.get_field('calib')
    horizon_state = vis_target.get_field('horizon_state').numpy()

    ### draw gt horizon(red)
    a, b, c = ground_plane[0], ground_plane[1], ground_plane[2]
    f_x, f_y, c_x, c_y = calib.f_u, calib.f_v, calib.c_u, calib.c_v
    # calculate the slope and intercept of the horizontal line on the original image
    F = -b
    K1 = (a * f_y) / (f_x * F)
    B1 = (c * f_y) / F + c_y - K1 * c_x
    B1 = B1 + pad_size[1] - K1 * pad_size[0]
    gt_horizon_img = cv2.line(horizon_vis_img.copy(), (0, round(B1)), (horizon_vis_img.shape[1], round(K1 * horizon_vis_img.shape[1] + B1)), (255, 0, 0), 2)

    ### draw LS pred_horizon(pink)
    pad_x = int(np.ceil(pad_size[0] / 4))
    v = np.argmax(horizon_heat_map[:, pad_x:horizon_heat_map.shape[1]-pad_x], axis=0)
    u = np.arange(pad_x, horizon_heat_map.shape[1]-pad_x)
    points = np.stack((u, v), axis=1)
    [vx, vy, x, y] = cv2.fitLine(points, cv2.DIST_L2, 0, 0.01, 0.01)
    K2 = float(vy) / float(vx)
    B2 = (-K2 * x + y)[0]
    B2 = 4 * B2
    gt_and_ls_horizon_img = cv2.line(gt_horizon_img.copy(), (0, round(B2)), (gt_horizon_img.shape[1], round(K2 * gt_horizon_img.shape[1] + B2)), (255, 192, 203), 2)

    ### draw GPEnet way pred_horizon(cyan)
    if horizon_state[0]:
        K3 = horizon_state[1]
    else:
        K3 = K2
    B3 = B2
    gt_and_GPEnet_horizon_img = cv2.line(gt_horizon_img.copy(), (0, round(B3)), (gt_horizon_img.shape[1], round(K3 * gt_horizon_img.shape[1] + B3)), (0, 255, 255), 2)

    ### draw ransac pred_horizon(yellow)
    ransac = linear_model.RANSACRegressor()
    ransac.fit(u.reshape(-1, 1), v)
    K4 = ransac.estimator_.coef_[0]
    B4 = ransac.estimator_.intercept_
    B4 = 4 * B4
    gt_and_ransac_horizon_img = cv2.line(gt_horizon_img.copy(), (0, round(B4)), (gt_horizon_img.shape[1], round(K4 * gt_horizon_img.shape[1] + B4)), (255, 255, 0), 2)

    GPEnet_K_error = abs(K3 - K1)
    LS_K_error = abs(K2 - K1)
    RANSAC_K_error = abs(K4 - K1)

    K_error = np.array([GPEnet_K_error, LS_K_error, RANSAC_K_error]).reshape(1, 3)

    max_coordinate = np.unravel_index(np.argmax(horizon_heat_map), horizon_heat_map.shape)
    max_coordinate = np.array(max_coordinate) * 4
    gt_and_ls_horizon_img = cv2.circle(gt_and_ls_horizon_img, (max_coordinate[1], max_coordinate[0]), 5, (0, 0, 255), -1)


    plt.figure(figsize=(12, 10))
    plt.subplot(311)
    plt.imshow(gt_and_GPEnet_horizon_img)
    plt.title('GPEnet')
    plt.axis('off')
    plt.subplot(312)
    plt.imshow(gt_and_ls_horizon_img)
    plt.title('LS')
    plt.axis('off')
    plt.subplot(313)
    plt.imshow(gt_and_ransac_horizon_img)
    plt.title('RANSAC')
    plt.axis('off')
    plt.show()
    ...
    # draw a scatter plot
    # max_horizon_heat_map = horizon_heat_map.copy()
    # max_horizon_heat_map[v, u] = 1
