    ...
    def forward(self, roi_points, bbox2d_pred, loss2d_feat, depth_pred, pred3d, bbox2d_gt, depth_gt, calibs):
        ...
        for i in range(batch_size):
            ...
            if mn_obj_cnt == 0:
                ...
                # 应将0替换某一正值, 且配置为参数
                loss_3d = loss_3d + \
                    (0 if (len(bbox2d_pred[i]) ==
                     obj_cnt_each_img[i]) else 500)
            else:
                # pdb.set_trace()
        ...
        total_loss = loss_3d * self.loss_weights[0] + depth_loss * \
            self.loss_weights[1] + bbox2d_loss * self.loss_weights[2]
        # pdb.set_trace()
        return total_loss, loss_3d, depth_loss, bbox2d_loss


class WDM3DDepthOffLoss(nn.Module):

    def __init__(self, model=None, sample_roi_points=100, dim_prior=[[0.8, 1.8, 0.8], [0.6, 1.8, 1.8], [1.6, 1.8, 4.]], loss_weights=[1, 0.9, 0.9], *args, **config) -> None:
        """
        sample_roi_points: 每个实例采样多少个点云中的点
        dim_prior: 
        loss_weights: 依次为3dloss, depth loos, 2d bbox loss的权重
    ...
    def forward(self, roi_points, bbox2d_pred, pred3d, bbox2d_gt, calibs, device=torch.device("cuda")):
        ...
        for i in range(batch_size):
            ...
            if mn_obj_cnt == 0:
                ...
                # pdb.set_trace()
                # print([len(r) for r in roi_points[i]])

                bbox2d = bbox2d_pred[i][:mn_obj_cnt]

                data, valid_instance_mask = generate_data_for_loss(
